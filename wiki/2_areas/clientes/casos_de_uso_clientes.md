...
**Particularidades / cronología:** abr-2026 factura USD 3.500 (abono) + USD 157,05 (Wallet service); FCI en discovery con Daxia (ene-2026), credenciales de PROD en curso (jun-2026). **2026-07-24:** avanza el bridge operativo del OB PJ rechazado en abril — mientras no exista automatización batch (ETA fin de septiembre 2026), el alta de cuentas comitentes pendientes (~100 hoy, hasta 1.400 en total) se hace manualmente a partir del ID de cuenta, con una herramienta de soporte en construcción (ver [`detalle_productos/onboarding/manuales_operativos.md §5`](../../3_recursos/detalle_productos/onboarding/index.md)). **2026-08-05 (reunión "OB PJ"):** el problema de fondo del OB PJ rechazado en abril se cuantifica — de ~120-250 intentos de alta, solo el 5% (12) resultó exitoso; el cliente tiene **2.000 personas jurídicas pendientes de dar de alta** y no se anima a salir a producción con el flujo actual (riesgo de pérdida del cliente). Se aprueba un nuevo flujo: un oficial de negocio de La Virginia (whitelist de 15 personas) carga la documentación en lugar del cliente final, con MVP comprometido a 1 mes (ver [`detalle_productos/onboarding/manuales_operativos.md §6`](../../3_recursos/detalle_productos/onboarding/index.md) y `tareas_producto.md` T-086). **2026-08-06:** el discovery de este rediseño se trackea como proyecto dedicado — ver [`proyecto-la-virginia-ob-pj`](../../1_proyectos/proyecto-la-virginia-ob-pj/proyecto.md). **2026-09-02 (reunión "Join Soporte Clientes"):** consulta sobre el estado del Onboarding Personas Jurídicas (PRD-223) — Matías Alzogaray se comprometió a confirmar si se mantiene la fecha de compromiso de salida a producción del **18 de septiembre** y a evaluar un ambiente de Staging para que el cliente valide antes del despliegue; se acordó realizar una demostración en Staging con el cliente entre el **14 y 16 de septiembre**, condicionada a que el desarrollo esté disponible en Staging el 14/09. Además, confirmado que el **endpoint de alta de cuenta comitente** (PRD-208, pedido explícitamente por este cliente) forma parte de la versión 73 (septiembre), con las tareas técnicas 1424 y 1425 confirmadas y 1423 en evaluación de inclusión. **2026-09-08 (mail "RE: Version W 73 Wallet Service", Fintexa):** confirmación formal a nivel de alcance de versión (no solo de palabra) — "el onboarding persiste el ID de solicitud en el KYC y se dispone un nuevo endpoint de alta de cuenta comitente que busca la información por esa solicitud", con **entrega a QA Externo el 21/09**, antes de la fecha de compromiso de producción del 18/09 — tensión de cronograma detectada pero no resuelta explícitamente en el mail. **2026-09-09 (reunión "Join Soporte Clientes", mismo día):** Mauro Suppan y Adriana Endzeliz repasan la fecha con Matías Alzogaray, quien **reafirma que el 18/09 se mantiene** (sin cruzar explícitamente contra el dato de QA del 21/09 del mail de Fintexa). Dos datos nuevos: el endpoint de cuenta comitente en desarrollo es **compartido con Copel** (mismo endpoint, Matías Alzogaray evalúa si entra en W73 o W74); y el **ambiente de staging ya está disponible y opera en paralelo** (Matías había dicho inicialmente que no, corregido en la misma reunión por Adriana Endzeliz y Emma Vignoles) — se acuerda comunicar los plazos con claridad al cliente. **2026-09-16 (reunión "Join Soporte Clientes"):** confirmado que el despliegue de la modificación de onboarding **no requiere cambio de dirección web ni de backoffice** — se notifica al cliente de la forma habitual, sin fricción adicional. **Fecha real de salida a producción: 17/09** (un día después del compromiso, por demora de Fintexa en comunicar a Soluciones Andinas la documentación de análisis de riesgo requerida — ver `direccion/iniciativas.md`, PRD-223). **2026-09-21 (reunión "Producto"):** pese a estar desplegado en producción, el **backoffice de operadores de personas jurídicas de La Virginia está sin uso activo** — un participante criticó severamente los problemas de usabilidad: la información de los legajos se presenta desordenada, dificultando la revisión del personal de cumplimiento. Se acordó documentar el proceso paso a paso (manual de usuario, ver [`detalle_productos/onboarding/onboarding_personas_juridicas.md §10`](../../3_recursos/detalle_productos/onboarding/index.md)) y evaluar mejoras de estructuración de datos con Cristian Bertoldi.
...
**Particularidades / cronología:** 11/06 Cuenta Remunerada en standby · 18/06 Bind autoriza a Octagon a usar su API de validación RENAPER desde OB propio · 25/06 se da de baja el servicio de Cuenta Remunerada (van con otro proveedor) · 02/07 Octagon migrará su OB a integración directa con RENAPER, dejando de usar el de Bind (requerirá re-validación PLD) · **19/08:** Bind hace una demo completa del flujo de onboarding propio (persona jurídica) a representantes de Octagon y al equipo de **Compliance de Banco Industrial** (banco vinculado a la operatoria de Octagon) — el equipo de Compliance de Banco Industrial pidió acceso directo a la plataforma de onboarding para auditar el legajo digital de los clientes sin solicitar la documentación manualmente, y Bind acordó otorgarlo. Se definió además avanzar en un "paquete de datos" para automatizar (post-aprobación del onboarding) la creación de la CBU y el alta del comercio en los sistemas de Bind — hoy ese paso es manual. Ver detalle técnico completo de la demo en [`detalle_productos/onboarding/onboarding_personas_juridicas.md §8`](../../3_recursos/detalle_productos/onboarding/index.md). **2026-09-02:** Octagon (Begoña Perez de Solay) envió el detalle concreto del paquete de datos propuesto para la integración del onboarding de personas jurídicas — incluye CUIT/razón social/legajo digital, datos del firmante, verificación en listas (sociedad/firmantes/beneficiarios finales), situación BCRA, domicilio fiscal/comercial, DDJJ cruzadas (FATCA/OCDE/PEP/UIF), estructura societaria (beneficiarios finales +10%) y validación de identidad de representantes contra RENAPER. Sin fecha de integración confirmada — el equipo técnico/producto de Bind (recién sumado por Diego Weledniger) debe diagramar la integración, y Luciano Dufain (Octagon) debe entregar acceso a la plataforma para que PLD se sume. **⚠️ 2026-09-08 al 14 — fraude confirmado en la extracción de efectivo por QR en cajeros: 556 transacciones, $239.466.000 ARS.** Reportado por Emma Vignoles (2026-09-15): al fallar la extracción (ej. monto superado por el límite mensual, o error físico al retirar billetes), Octagon genera automáticamente una devolución (DCT) por el **total** de la operación hacia la billetera iniciadora. **Bug explotado:** cuando el cliente retiraba el efectivo pero dejaba **1 solo billete dentro del cajero** (retiro incompleto, no falla total), el sistema igual interpretaba la operación como fallida y devolvía el 100% del monto — el cliente se quedaba con casi todos los billetes retirados físicamente **y además** recuperaba el saldo completo en la billetera. Billetera iniciadora: **MELI, 100% de los casos**; 84 clientes/CUITs iniciadores, 44 ATMs involucrados; Octagon apagó el servicio el 14/09 al detectarlo y está armando la denuncia penal. **Acciones de Bind:** denuncia a la CPF (Central de Prevención de Fraude de Coelsa, Nicolás Colón); aviso a MELI para bloquear a los clientes involucrados y los 84 CUITs a black list (Rocío Revelli). Adjunto no descargado: `Casos_con_Diferencia_de_Billetes_QR.xlsx`.
...
**Particularidades / cronología:** múltiples acuerdos a firmar (Marco, Sign Letter, Aagi, PSP/IVSA, Bind UY) con distintas sociedades del grupo Inter — requiere revisar con cuál firma cada acuerdo. **2026-08-31 — Incidente de producción, onboarding con caída sustancial de aprobación:** desde el 2026-08-26 la tasa de aprobación de altas de onboarding cayó de **85% a 32%**, con **352 rechazos en los últimos 3 días** por el error "No se pudo encontrar el PDF417 en las imágenes procesadas" (el problema venía arrastrándose "hace más de un mes" según Emma Vignoles, agravado fuertemente desde el 26/08). Contexto crítico: Inter tenía programado el comunicado de prensa de su app para el 2026-09-03 — resolución urgente por impacto de imagen/negocio, no solo por volumen de rechazos. Causa raíz sin identificar al 2026-08-31: se descartó que faltara el dato "Género" (opcional para Inter); el problema real es la interpretación del PDF417 de la imagen del DNI que Bind PSP debe hacer server-side (Inter integra por API completa, sin pasar por el front de Onboarding). Hipótesis sin confirmar: correlación con el DNI argentino nuevo, sin PDF417 (ver `1_proyectos/prd-113_leer_nuevo_dni/` si existe, o `detalle_productos/onboarding/`). Seguimiento en `1_proyectos/tareas.md` T-052. **2026-09-21 (reunión "Producto"):** Bind PSP acuerda **empezar a cobrar a Inter por cotizaciones y desarrollos de funcionalidades personalizadas** fuera del alcance estándar — antes esto no estaba formalizado (ver decisión general en [`direccion/decisiones.md`](../direccion/decisiones.md) [2026-09-21]); Mauro queda en redactar el proceso de cobranza. Cuatro pedidos activos de Inter en la misma reunión: (1) **reprocesamiento masivo de verificaciones manuales** — 180 solicitudes acumuladas en onboarding, imposibles de procesar una por una; Nicolás Colón confirmó viable agregar reprocesamiento masivo al frontend, cotización final **70 horas** (ver proyecto `cola_verificacion_manual`/PRD-242 en `direccion/iniciativas.md`, cotización de Fintexa de 56hs por separado — discrepancia sin resolver); (2) **columna de motivo de error** en el alta de cuentas del broker — cotización sube a **40 horas** al sumar visualización + exportación a Excel; Alan (comercial) había indicado que Inter no pensaba pagarlo, se ratificó que si no acepta el pago no se hace el desarrollo (ver proyecto `visibilidad_error_alta`/PRD-243); (3) **cuenta comitente en dólares** — Inter propone comprar y transferir dólares para retenerlos en una cuenta comitente, preocupación por la complejidad de integrar con IPSA, sin alcance ni estimación todavía, falta asignar a alguien de Producto para este frente; (4) **integración de legajos vía API** — Inter exige compartir el 100% de los legajos de sus ~10.000 cuentas de clientes existentes; como Cumplimiento no autoriza legajos incompletos, Bind está construyendo una API para que Inter envíe la documentación y validaciones completas del onboarding directamente, a revisar con Cumplimiento. Nicolás Colón y Alan tienen reunión semanal con Inter para estos temas, sin fecha puntual.
...
**Pricing:** QR 0,6%.
**Particularidades / cronología:** **2026-08-31 (reunión "Demo Mayoristas"):** demo completa (onboarding, POS, billetera, backoffice) del ecosistema "eco cerrado" para mayoristas, con Maxiconsumo como uno de los participantes centrales. Novedad de negocio: Eugenio Gigena y Emma Vignoles confirmaron **3 socios definidos para la billetera** — "Yaar Vital" (nombre no confirmado, no tiene ficha propia — posible variante/error de transcripción de otro cliente ya cargado) y Maxiconsumo, con posibilidad de sumar más participantes como "adherentes" a futuro. Acuerdo privado en curso, cierre estimado **septiembre 2026**. Ver `1_proyectos/comercios_mayoristas/` (proyecto de Pablo Gomes, condicionado a este cierre) y `direccion/iniciativas.md`.
...
**Particularidades / cronología:** es el modelo de referencia ("modelo Depay") que luego replicó Inswitch para sus clientes brasileros. **2026-09-04 (mail "Re: Depay/Bind PSP- Tecnica"):** en el marco del seguimiento técnico de tiempos de Pagos QR, Depay (Paloma Menendez Palau) avisó el volumen esperado para su producto de **cobros con QR: ~100.000 transacciones mensuales**, con incremento escalonado del tráfico en las próximas semanas hasta llegar a ese nivel — pidieron que Bind lo tenga en cuenta por si requiere pruebas o validación de capacidad de su lado.
...
- **2026-08-19 (mail "2nd Factor Wallet", Gonzalo Santos/Credicuotas):** el cliente pidió visibilidad de fecha estimada sobre un pedido pendiente de análisis desde la reunión homónima del 2026-08-18 (equipo Bind PSP: Nicolás Colón y "Ro", con Facundo Aguirre de Poincenot y Rodrigo Revelli) — una solución de **segundo factor de autenticación combinada con diferenciación de créditos**, para poder setear nuevos límites de esa operación en Wallet. Sin definición de producto ni ticket de Jira propio a la fecha de esta captura.
...
**Particularidades / cronología:** **2026-09-03:** reclamo por no poder ejecutar contracargos/devoluciones desde el portal admin, por un error de timeout (ticket AD1639) — levantado con carácter urgente explícitamente por tratarse de "un cliente de toda la vida" (cliente histórico). Fix desplegado a producción el mismo día, en horario laboral, validado previamente en staging (ver `detalle_productos/adquirencia/` para el detalle técnico del fix de query id/referencia). Este incidente también motivó el debate sobre criterio de clasificación hotfix vs. despliegue estándar — ver `procesos/analisis_de_riesgo_de_despliegue.md`.
...
- **2026-08-21 (reunión "Análisis de riesgos AD V72"), ⚠️ alerta abierta, no confirmada como resuelta:** al revisar el cambio de categorización de transferencias entrantes de CBU externo→CBU corto en CBU Collect (pasan a notificarse como `transfer.cortau` en vez de CBU largo — ver [`detalle_productos/adquirencia/webhooks_y_notificaciones.md`](../../3_recursos/detalle_productos/adquirencia/index.md)), Matias Alzogaray identificó a Jugadón como cliente que podría verse afectado si filtra/valida por ese campo de tipo en el webhook. Se acordó evaluar el impacto puntual y avisarle con anticipación antes del pase a producción del 27/08 (AD V72) — a la fecha de esta captura no hay confirmación de que la evaluación/aviso ya se haya hecho.
...
- **⚠️ 2026-09-10, 9:14hs — fraude confirmado en la app móvil: transferencias salientes no autorizadas, 217 clientes afectados.** Reportado por Eduardo Dezuliani (Director de Legales/Compliance/RRHH de BSF, 2026-09-16): detección de transacciones no autorizadas que extrajeron fondos de 217 clientes hacia cuentas de terceros. Los servicios de la app se restablecieron parcialmente desde las 15hs del mismo día (con intermitencias por cambio en el proceso de validación de logueo) y con normalidad total desde las 19:30hs. **Acciones correctivas de BSF:** reintegro de fondos a los 217 clientes; piso de validación biométrica en transferencias salientes a terceros bajado de $500.000 a **$50.000**; validación biométrica (rostro) agregada al recupero de usuario/contraseña; notificación a los clientes afectados; denuncia penal presentada. BSF seguía analizando la causa raíz sin confirmarla por escrito — Bind PSP la pidió explícitamente y quedó sin respuesta a la fecha de este reporte. **Acción de Bind (2026-09-16, Emma Vignoles a Rocío Revelli/Nicolás Colón):** denuncia a la CPF (Central de Prevención de Fraude de Coelsa) — mismo patrón que el incidente de Octagon del mismo mes (ver ficha OCTAGON). Adjunto no descargado: `Clientes afectados 100926.xlsx`.
...
- **2026-08-21 (reunión "Análisis de riesgos AD V72"):** el ticket 1458 (lote de despliegue AD V72, 27/08/2026) corrige un caso donde no se debitaban correctamente en las cuentas de Coto los impuestos QR, por configuraciones previas en los comprobantes de Wallet. Matias Alzogaray y Maria Eugenia Vila lo clasificaron con **semáforo amarillo** (no verde, pese a ser un fix acotado) explícitamente por la criticidad del impacto en Coto — requiere controles postimplementación dedicados; Maria Eugenia queda a cargo de controlar el cálculo de impuestos QR/Wallet en base de datos tras el despliegue.
...
**Particularidades / cronología:** **2026-09-02 (reunión "Join Soporte Clientes"):** aumento significativo de rechazos de tarjetas de débito y crédito. Causa identificada por Adriana Endzeliz: no es una regresión nueva, sino el efecto esperado de la corrección de un bug de grupo BIN mal configurado en Ardid (ver `detalle_productos/ardid/`) — las reglas de validación de pago del cliente ahora impactan correctamente por primera vez, rechazando transacciones que antes pasaban sin control. Reglas de validación involucradas (transversales, no exclusivas de este cliente): límites de $300.000 por transacción y $2.000.000 mensuales. Rocío Revelli (junto con Gonzalo Rivera) analiza el impacto y ajusta parámetros para reducir la fricción sin resignar seguridad — sin fecha de resolución confirmada; próximos pasos: corregir el error técnico causante del alto índice de rechazos, repasar todas las reglas de entidad vigentes, y evaluar configuraciones de reglas por entidad.
...
**Particularidades / cronología:** dic-2025 — no avanzan con el Onboarding de Bind (van con otro proveedor); jun-2026 — integran QR Aceptador, prueban DEBIN nuevos y reactivación de suscripciones OK, lanzamiento en redes; interés futuro en Agente de CyP; plan: evolucionar a FCI cuando CA le dé una wallet a cada unidad funcional. **2026-09-09 (reunión "Join Soporte Clientes"):** Mauro Suppan expuso la cotización presentada a Consorcio Abierto para el pago de expensas en efectivo mediante **redes extrabancarias**: cuenta exenta con comisión de **0,3% de entrada y 0,3% de salida**. Advirtió que redes como Ripsa o Cobro Express cobran un recargo de **2% a 4%** por la recepción de efectivo — costo que el cliente todavía no contempló en su análisis. Comparado contra Rela (competencia, cobra 5% a los consorcios). Se acordó contactar a Diego Weledniger y a Ripsa para negociar tarifas y evaluar eximir la comisión de salida en la cuenta virtual (Mauro Suppan a cargo de alinear con el cliente).
...
**2026-08-18 (reunión "Producto", confirmado por Pablo Gomes):** el error reportado por Arcos Dorados de productos que no aparecían en el orden de venta correcto al leer el código QR ya fue corregido en staging, con pase a producción esperado para la semana del 24/08.
**2026-09-09 (reunión "Join Soporte Clientes"):** la **fecha de salida a producción se corrió de septiembre a octubre** por retrasos en el desarrollo (confirmado por Emma Vignoles) — Adriana Endzeliz había solicitado el flujo de validación de DNI con datos de APD para el cierre de cuenta, pero las reuniones de definición quedaron suspendidas temporalmente por definiciones internas. **Riesgo de seguridad aceptado:** Romy (Seguridad) impulsó un nuevo flujo para dar de baja la cuenta y salir a producción, dejando la implementación completa del onboarding para después — Seguridad calificó esto como riesgo, pero Emma Vignoles cuestionó la postura (la baja puede hacerse manualmente) y se acordó avanzar igual con un **formulario manual** de baja.
...
**Particularidades / cronología:** fix de collector RxT por CUENTA+SUBCUENTA (abr-26) para PSP propia; reclamo de performance en lectura/pago QR (orquestación PSP Global66 + PSP 184); interesados en tarjetas de crédito, agente de cobros en dólares, monitoreo transaccional (adenda en curso). 14-jul-2026: identificado como el cliente de **mayor impacto de tráfico sobre la infraestructura base** — ráfagas de más de 9.000 comprobantes/minuto que presionan la base de datos compartida; el equipo técnico evalúa contactarlo para pedir que distribuya el envío de comprobantes en vez de enviarlos en ráfaga, y definir rate limits (ver `wallet/otros_manuales.md §11` y tarea T-011). **2026-08-05 (reunión "Bind Global66, proximos pasos"):** el 20-jul-2026 se registraron 732 transacciones QR con 29% de rechazo (212), concentradas después de las 19hs, por inconsistencias de infraestructura con APIBanco/Coelsa durante julio — se abre ticket para pedir postmortem formal. Cliente evalúa homologar **billetera interoperable** (QR interoperable, cotización aparte del contrato vigente, ~2-3 meses de proceso) y **onboarding de personas jurídicas** vía Global (mismos endpoints que PF, documentación de compliance específica ya provista) — Cisc (motor impositivo) ya contempla PJ. Cambio técnico simplificador evaluado a futuro: operar débitos/créditos directo desde la CVU de BM PCP en vez de la cuenta de pagos actual. **2026-09-02 (reunión "Bind Global66, proximos pasos"):** avance clave en el reclamo de latencia de Pagos QR (proyecto `bajar-tiempos-pagos-qr`, PRD-199) — se acota la causa de la discrepancia de medición: Bind mide de forma consistente ~6,7-7s hasta la actualización de estado (validado en vivo, 6,8s exactos contra logs de infraestructura), mientras Global66 mide 15-20s del lado usuario. Al comparar timestamps de una misma operación, Marco Narváez (Global66) reconoció que su medición arranca **desde que el usuario presiona el MFA** para autorizar el pago, no desde que la petición llega a Bind — la latencia adicional probablemente ocurre en el propio flujo de Global66, antes de llegar al servidor de Bind; queda como tarea de Global66 revisarlo internamente. **Hallazgo nuevo, separado:** casos de transferencias entrantes que figuran como existentes en el extracto pero no quedan acreditadas en el sistema de Global66 en el momento, pese a que Bind confirma haber enviado el webhook correcto — Alejandro López Torres (Global66) queda a cargo de revisar ese flujo la semana siguiente; hipótesis de Bind (Nicolás Colón/Gonzalo Rivera): podría no estarse insertando la transacción al recibir el webhook, sino recién en la consulta posterior. Distinto del hallazgo ya trackeado en `1_proyectos/tareas.md` T-018 (mapeo erróneo de transferencias salientes como recibidas, Agente de Cobros y Pagos) — mismo dominio, síntoma distinto, a confirmar si comparten causa raíz. Reunión de seguimiento agendada para la semana del 2026-09-08.
...
## Provincia Net
> Estado: sin ficha propia en Notion/`log_clientes.md` todavía — cliente/integrador real, mencionado extensamente en el canon técnico y operativo pero nunca relevado por `/sync_customers`. Contacto: Facundo Nicolás Collerone (Gerencia de Pagos Digitales, Provincia Net).

**Modelo de negocio:** integrador de cobro masivo de terceros (municipios, empresas de servicios) contra la API de creación de deuda/QR de Bind — es el mayor generador de Deudas del sistema (2.563.059 en 98 días, 2,5× la entidad #2), con un segmento de crecimiento proyectado hacia clientes individuales/interactivos (telefonía, transporte). Ver el detalle técnico completo del incidente de saturación de cola QR en [`detalle_productos/adquirencia/incidente_qr_masivo_provincia_net.md`](../../3_recursos/detalle_productos/adquirencia/incidente_qr_masivo_provincia_net.md) y el seguimiento de mitigación en [`1_proyectos/prd-66_provincianet_creacion_masiva_qr/proyecto.md`](../../1_proyectos/prd-66_provincianet_creacion_masiva_qr/proyecto.md).
**Particularidades / cronología:** **2026-09-10** — Provincia Net compartió video + transcripción de una reunión con Bind PSP (mail de Facundo Collerone, vía WeTransfer, link único y vencible) para tratar performance/recurrencia del canal. El contenido **nunca se pudo revisar**: no es descargable automáticamente desde WeTransfer y el link no fue abierto en ninguna sesión posterior — la minuta formal que Facundo Collerone había ofrecido "al final del día" tampoco se recibió. Se documenta acá la existencia de la reunión y el pendiente de revisión manual, no su contenido (desconocido). Si en algún momento se abre el link o llega la minuta, capturar como item nuevo de `contexto_vivo/` (probablemente vencido a esta fecha — pedir un reenvío si todavía hace falta el contenido).
