...
### 2026-09-14, segunda corrida (pablo) — corrida automática (scheduled task, sin usuario presente)

- `direccion/iniciativas.md` — upsert de la fila PRD-216 (Arcos Dorados): novedad de auditoría de cierre (`/idea_golive`) — desarrollo en producción desde el 31/08 pero la IDEA madre sigue en "Shipping", con 4 pendientes de cierre formal (aprobación de Dirección, reaviso de fecha al cliente, validación nueva no documentada en el alcance, 2 defectos colaterales de webhook no bloqueantes).
- `gaps_y_preguntas.md` — actualizada la entrada de `dim_collectors` (cuarta reconfirmación consecutiva del mismo mapeo, semana 202637) y 1 gap nuevo: Tarjeta Prepaga (Payway) con dos semanas consecutivas de crecimiento explosivo + rechazo muy por encima de lo habitual, sin causa confirmada.
- Items en régimen D/sin clasificar de corridas anteriores: sin cambios, siguen `en_cola` (ver manifiesto `2026-09-14.md`, sección de la segunda corrida).
