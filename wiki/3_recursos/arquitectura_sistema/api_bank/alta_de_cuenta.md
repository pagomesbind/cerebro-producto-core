# API BANK — Alta de Cuenta (onboarding PSI/STI)

> ⚠️ No usado hoy por Bind — Bind opera contra API BANK principalmente para Cuenta/Billetera/Transferencias (ver [cuenta.md](cuenta.md), [billetera.md](billetera.md)), no para onboarding de cuentas nuevas vía PSI/STI. Documentado como referencia por si en algún momento se evalúa este flujo.
...
| `body.civil_status` / `body.sex` / `body.occupation` / `body.occupation_description` / `body.laboral_activity` / `body.laboral_activity_description` / `body.nationality` | String | Sí | Códigos definidos en un "Archivo de Códigos-Descripciones" externo al portal (ver **gap** al final de este documento). |
...
| `accounts[]` | Object[] | Detalle de cuentas creadas — `account_id`, `cbu`, `currency`, `token`, `expiration_date`, `account_active`. |
...
Renueva el token de operación (`accounts.token` obtenido en el alta) de un cliente PSI/STI — necesario para seguir operando la vista `delegate` (ver [vista.md](vista.md), header `Delegate-authentication`).
...
## ⚠️ Gap — catálogo de códigos PSI/STI no accesible desde el portal público

Varios campos obligatorios del body de `PSICrearCuenta`/`STICrearCuenta` (`civil_status`, `sex`, `occupation`/`occupation_description`, `laboral_activity`/`laboral_activity_description`, `nationality`, `province_code`/`province`, `country`) remiten a un catálogo de códigos externo al portal, referenciado en la descripción del endpoint como:

> "En el documento adjunto se definen los valores posibles... Archivo de Códigos-Descripciones (`@@URL_PSI_CODE_DESC`)"

El placeholder `@@URL_PSI_CODE_DESC` no está resuelto en el JSON fuente (`api_data.json`) — es una variable de template de apidoc.js que normalmente se sustituye en tiempo de build o requiere navegar el portal con JS para resolverse a una URL real. No se pudo acceder al contenido de ese archivo desde el relevamiento (basado en fetch directo del JSON del portal, sin ejecución de JS de la página).

**Impacto:** `PSICrearCuenta`, `STICrearCuenta` y `STICrearCuentaPJ` quedan documentados con la forma de sus parámetros pero **sin los valores concretos permitidos** para varios campos obligatorios. Limita el valor de esta documentación como referencia de implementación si en algún momento Bind necesitara integrar directamente contra estos endpoints — hoy no se usan.

**Cómo resolverlo, si hiciera falta:** navegar el portal con un navegador real (ejecutando JS) para ver si el link se resuelve, o consultar directamente a Banco Industrial/Fintexa por el "Archivo de Códigos-Descripciones" de la integración PSI/STI. No es urgente mientras Bind no tenga un proyecto activo de onboarding vía PSI/STI de API BANK.

## Nota de relevamiento

Grupo completo (5/5 endpoints relevados) desde `https://sandbox.bind.com.ar/apidoc/api_data.json`.
