# API BANK — Transferencia

> Grupo **Transferencia**: transferencias salientes desde una cuenta "normal" del banco (no específicamente desde un CVU de billetera virtual — para eso está el grupo [Billetera](billetera.md) / `TRANSFER-CVU`). Header `Authorization: JWT :token` obligatorio en todos. `bank_id` siempre `322`, `view_id` siempre `"owner"`.
...
Grupo completo (4/4 endpoints relevados) desde `https://sandbox.bind.com.ar/apidoc/api_data.json`. Igual que en Billetera, varios campos referencian catálogos del grupo `Referencias` (pendiente de relevar).
