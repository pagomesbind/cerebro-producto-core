...
> Grupo **Debin**: Débito Inmediato (DEBIN), mecanismo por el cual un vendedor solicita el cobro de un importe a un comprador, que debe confirmarlo (o tiene una recurrencia preautorizada). Relevante para el dominio de cobros de Agente de Cobros y Pagos de Bind. Header `Authorization: JWT :token` obligatorio en todos. `bank_id` siempre `322`, `view_id` siempre `"owner"`.
...
Grupo completo (7/7 endpoints relevados) desde `https://sandbox.bind.com.ar/apidoc/api_data.json`. El endpoint `ConsultaVendedor` viene con descripción y detalle de response vacíos en la fuente — no es un límite de este relevamiento, es así en el portal. Catálogos referenciados (`Currency`, `ConceptoTX`, `EstadoTX`, `EstadoDEBIN`, `OperationType`) pertenecen al grupo `Referencias`, fuera del alcance de esta ronda.
