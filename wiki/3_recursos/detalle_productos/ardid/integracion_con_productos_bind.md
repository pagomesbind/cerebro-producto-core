...
### 14.5 Confirmación de alcance en producción — qué entra en W73 y qué queda afuera (2026-09-08)

Fintexa confirmó (mail "RE: Version W 73 Wallet Service", 2026-09-08) qué parte de este discovery entra efectivamente en la versión **W73** de Wallet, y qué queda afuera — la primera vez que alguna de estas 3 piezas pasa de discovery a alcance de versión con fecha:

- **Dentro de W73:** mapeo del motivo de rechazo (cierra §6.1); habilitación global de operaciones (§14.1); **rechazo de todas las operaciones cuando Ardid está caído o no responde** (§14.2); deshabilitación automática de cuentas bloqueadas.
- **Queda afuera, pasa a W74:** el **state monitor** (§14.3) — "se sigue analizando; no bloquea al resto de los desarrollos de Ardid".

**Riesgo funcional explícito señalado por el propio Fintexa:** habilitar el rechazo global (§14.2) **sin** el state monitor que hoy lo contendría (§14.3) tiene impacto funcional — puede rechazar transacciones legítimas durante una caída de Ardid, no solo las fraudulentas. Es precisamente la secuencia que §14.3 buscaba evitar (encolar altas/entrantes, rechazar salientes solo tras pasar por análisis) — al salir 14.2 sin 14.3, ese colchón todavía no existe. Fintexa se comprometió a avisar con anticipación a Soporte y hacer un análisis de riesgo antes de habilitarlo en producción. Riesgo a monitorear antes del despliegue de W73 (entrega a QA Externo 21/09).
...
Al analizar un fix de Pentass para corregir estados de transacciones, surgió un segundo problema más grave: dentro de Ardid, uno de los filtros de reglas de pago es por **grupo BIN**, y en la entidad afectada ese filtro estaba configurado únicamente con la opción **"todos" sin ningún grupo BIN específico asignado**. Como consecuencia, **las reglas de monitoreo de pago nunca estaban impactando** — si una transacción se rechazaba, era por otro motivo, nunca por estas reglas. El equipo (Rocio Revelli y Lorena Macedo, Fintexa) detectó que el mismo fix de Pentass que corrige el estado de la primera transacción de una tarjeta (cuando Ardid no encuentra el hash de 16 dígitos asociado, mostrada hoy como "pendiente" sin reflejar el estado real) también resuelve este problema de grupo BIN.

**Detalle del fix relacionado:** paquete de Pentass sin necesidad de compilación (imágenes Docker de API de transacciones + servicio de transacciones, actualizador de base de datos SQL Server/MongoDB, manual). Decisión: desplegar primero en Staging para regresión de QA antes de producción. Se pospuso la implementación para no interferir con el pase de AD V72 (compromiso de cliente, 38-40 historias de usuario esa misma noche); la migración de base de datos podría requerir una ventana de mantenimiento de ~1 hora en producción (en Fintexa tardó <5 min, pero con menor volumen de datos que Bind PSP) — a coordinar con el cliente **Coto**, que no puede ser "bypaseado" en la ventana nocturna.

**Reaparición (reunión "Priorización de despliegues", 2026-08-28):** Mariana Nadalin señaló que "no estaban haciendo validaciones de las reglas" desde la semana anterior, generando alto rechazo en Payway, y que el fix inicial del 26 de agosto estaba mal y requirió correcciones adicionales vinculadas al ID de comercio.

## 16. Confirmación en producción — el fix de grupo BIN (§15) ya está generando rechazos legítimos nuevos (cliente Terramundi, 2026-09-02)

> Fuente: Reunión "Join Soporte Clientes" (2026-09-02), minuta Gemini.

Primer caso concreto observado del efecto esperado del fix de grupo BIN documentado en §15 (filtro mal configurado que hacía que las reglas de pago **nunca** impactaran): Diego Gaston Weledniger (Comercial) reportó un aumento significativo de rechazos de tarjetas de débito y crédito para el cliente **Terramundi (Peak Travel)** — ver ficha en `2_areas/clientes/casos_de_uso_clientes.md`. Adriana Endzeliz confirmó la causa: no es una regresión nueva, es la corrección de §15 impactando correctamente por primera vez — transacciones que antes pasaban sin ser evaluadas por el bug ahora se rechazan si no cumplen las reglas vigentes de la entidad.

**Reglas de validación concretas involucradas** (transversales, no exclusivas de Terramundi): límites de **$300.000 por transacción** y **$2.000.000 mensuales**. Rocío Revelli quedó a cargo de: (1) corregir un error técnico adicional en las reglas de Ardid que está agravando el índice de rechazos, (2) repasar todas las reglas de entidad vigentes, y (3) evaluar configuraciones de reglas por entidad — en conjunto con Gonzalo Rivera, quien compartió una consulta de base de datos ad hoc para facilitar el monitoreo de rechazos por entidad y marca (limitación operativa señalada: hoy es difícil identificar, dado un rechazo puntual, cuál regla específica lo causó). Objetivo: ajustar los parámetros de las reglas para reducir la fricción de UX del cliente sin resignar seguridad — sin fecha de resolución confirmada a esta fecha.

**Lectura:** confirma en producción, con un caso real de cliente, la hipótesis de impacto que ya se anticipaba en §15 al desplegar el fix — el aumento de rechazos es la corrección funcionando como se esperaba, no un nuevo bug.

## 17. Segmentación de personas jurídicas por CUIT — brecha de identificación automática (discovery, 2026-09-09)

> Estado: discovery — no construido. "Requiere más debate", sin decisión ni desarrollo iniciado a esta fecha.

> Fuente: Reunión "ARDID" (2026-09-09, 15:30), minuta y transcripción Gemini.

En el marco del nuevo límite operativo para cuentas de personas jurídicas (ver gap sobre el monto exacto en el `gaps.md` del proyecto de onboarding correspondiente), surgió que **el sistema no tiene hoy un método automático para distinguir si una CBU/CVU corresponde a una persona jurídica**: las cuentas de personas jurídicas se registran bajo el mismo segmento ARDID que las personas físicas, pese a tener CUIT (en vez de CUIL).

**Propuesta técnica (Nicolás Colón):** adaptar el alta de cuentas en Wallet para identificar personas jurídicas cuando el CUIT comienza con el prefijo **"30"**, y usar esa identificación para asignarles un segmento distinto en Ardid. Mariana Nadalin (Ardid/Akurtech) coincidió en que sería la mejor opción, pero aclaró que **hoy esa integración no existe**: Wallet tiene la lógica de identificación de CUIT pero no actúa directamente sobre Ardid — mismo patrón de brecha "Wallet sabe algo que no le informa a Ardid" ya visto en otros puntos de este documento (§5, §6).

**Gestión de cuentas existentes vs. nuevas:** para las cuentas de personas jurídicas ya creadas, se puede migrar manualmente creando un nuevo segmento en Ardid; pero gestionar las cuentas **nuevas** de esta forma requeriría consultas diarias manuales a la billetera — calificado como inviable a escala por Mariana Nadalin.

**Próximos pasos:** Nicolás Colón hablará con el equipo de Wallet sobre la identificación automática de personas jurídicas en el alta, y consultará una alternativa más directa de segmentación. Sin decisión ni ticket a esta fecha.

**Tema aparte, mismo encuentro:** durante la revisión conjunta de reglas de transferencia salientes/entrantes por tipo de cliente y empresa, Mariana Nadalin advirtió posible desacuerdo de Matías Alzogaray con la modificación de prioridades que esto implica; se acordó igualmente abrir un ticket en PMC.

## 18. Identificación de transacciones ante Ardid por `PaymentId` de Botón Simple, no por el ID interno de Cobro (2026-09-17)

> Fuente: sesión `/idea_solution` sobre `ardid_desconocimientos` (PRD-248), aporte directo del PM.

Al diseñar cómo informarle a Ardid que una transacción de Botón Simple tuvo un contracargo tipo "desconocimiento" (`POST /api/FilProcess/Process`, campo `transactionId`), surgió una precisión que no estaba explícita en el resto de la integración ya documentada: **el identificador que Ardid espera no es el `transaccionId` interno del sistema de Cobro** (el que aparece, por ejemplo, en el request del endpoint de refund/contracargo, ver [`devoluciones_y_contracargos.md §0`](../adquirencia/devoluciones_y_contracargos.md)) — **es el identificador con el que Ardid ya conoce esa transacción desde que la analizó por primera vez** (llamada original a `/Transaction`/`Analyze`).

Para el canal Botón Simple, ese identificador es el **`PaymentId`** (el id del link de pago) — consistente con lo ya documentado en el catálogo de webhooks de contracargo de Adquirencia, donde `IdentificadorOrdenVenta` "para Botón simple es el PaymentId (id del link de pago)".

**Implicación práctica para cualquier integración futura que necesite referenciar ante Ardid una transacción ya analizada:** no alcanza con el ID interno de Cobro/liquidación — hay que resolver primero el `PaymentId` (u otro identificador equivalente según el canal, ver la misma tabla de `IdentificadorOrdenVenta`/`IdOrdenVentaQR`) antes de armar el llamado a Ardid. El mecanismo exacto para resolver ese identificador a partir de los datos que circulan en el flujo de contracargos (`deudaId`/`transaccionId`) quedó delegado al equipo de desarrollo tercerizado como detalle de implementación — no se relevó en esta sesión. Usado en el diseño de la iniciativa `ardid_desconocimientos` (ver [`2_areas/direccion/iniciativas.md`](../../../2_areas/direccion/iniciativas.md)).

## 19. Cómo crea hoy Wallet la segmentación en Ardid — `BankType`/`ClientBankType` reales y propagación del cambio de segmento (2026-09-23)
...
**Creación de la segmentación de una Organización en Ardid, tal como la hace hoy Wallet:**

1. **Primero el tipo de banca (`BankType`), uno propio por Organización/Entidad** — nunca compartido. Se crea con dos query params, sin cuerpo: `Name` y `entityCode` (catálogo del proveedor: `POST /CreateBankType`, API `/BankTypes`). Wallet **guarda el Id que devuelve** y con ese Id crea el `ClientBankType` consecuente — uno nuevo cada vez.
2. **Después los niveles (`ClientBankType`)**, con este cuerpo real (camelCase):
   ...
   ```
   ⚠️ **No se envía `ClientTypeId`**, pese a que el catálogo documentado del proveedor lo marca como obligatorio (ver [`apis_externas.md §10.a`](apis_externas.md) — anotado el 2026-09-25 con esta discrepancia, y gap abierto en [`2_areas/gaps_y_preguntas.md`](../../../2_areas/gaps_y_preguntas.md)). `personTypeId` va en `1` para todos los tipos de titular. `description` = `name`. `externalClientTypeId`: `0` persona física adulta, `1` persona física menor (en producción desde PRD-17/WS-53).
...
**Nombres vigentes hasta hoy** (Organizaciones existentes): `Standard {Entidad}` (persona física adulta) y `Standard {Entidad} menor` (persona física menor), tanto para el `BankType` como para su `ClientBankType`. El proyecto `ardid_limites_pj` define una convención nueva para Organizaciones nuevas (`Persona Fisica Mayor/Menor {Entidad}`, `Persona Juridica {Entidad}`, con variante `... Restringido {Entidad}`) — se incorporará al canon cuando el proyecto cierre (ver [`2_areas/direccion/iniciativas.md`](../../../2_areas/direccion/iniciativas.md)).

**Cambio de segmento de una cuenta:** la operación de WalletCuenta `PATCH /api/v1/Cuenta/{id}/ActualizarSegmento` (header `x-entidad`, body `{"segmentoId": N}`, donde el segmento es el del Calculador de Costos) **ya pasa el cambio a Ardid para cualquier cuenta de Wallet**, siempre que el segmento del Calculador de Costos esté enlazado a su `ClientBankType` equivalente (campo `ardidClientBankTypeId` del segmento). No distingue persona física de jurídica. Los segmentos del Calculador de Costos se crean con `{tipo, codigo, descripcion, ardidClientBankTypeId, entidadIdExterno}`, y `tipo`/`codigo` tienen que ser únicos en toda la base cada uno por separado (columna `nvarchar(MAX)`, probado por el PM). **Nota:** este mecanismo ya cubre el cambio puntual de segmento restringido→estándar que necesita `ardid_limites_pj` — lo que falta ahí no es la capacidad técnica (ya existe), sino el proceso operativo de quién ejecuta el cambio y con qué autorización (sin responsable formal a la fecha, ver `iniciativas.md`).

---
*Fuente: Notion histórico, Epics "Ardid para wallet: Transferencias" (21 tickets), "Ardid para botón simple MVP" (11 tickets), "Ardid - Bóveda: integrar" (3 tickets, 7 SP), "Reparar Ardid+Wallet: Alta de cuentas y de segmentos" (11 tickets, 50 SP), "Ajustes Ardid+Wallet: Informar datos de localización en operaciones" (1 ticket, 20 SP, discovery) y "Ajustes Ardid+Botón: Informar localización e informar devoluciones" (3 tickets, 15 SP, discovery) — ingesta 2026-07-06. §7: Jira `bindpsp.atlassian.net`, IDEA PRD-115 + Epic WS-548 (3 Historias: WS-549, WS-550, WS-551) — ingesta 2026-07-06. §8-10: backfill `/sync_releases` vía export XML, 2026-07-13. §11: `/sync_meetings`, reunión 2026-07-17. §11.1: `/sync_mails`, mail "Restauración Contadores Mensuales", 2026-07-20. §12: `/sync_meetings`, reunión "Análisis de riesgos - W 71.6", 2026-08-05. §13: `/sync_meetings`, reunión "Productos - Weekly Seguimiento", 2026-08-11. §6.1: `/sync_meetings`, reunión "Producto", 2026-08-18. §14: transcripción manual en sesión libre (no vía `/sync_meetings`), reunión "Revisión Wallet - BIND", 2026-08-21. §15: `/context_merge` desde `contexto_vivo/`, reuniones "FIX - Pagos" (2026-08-26) y "Priorización de despliegues" (2026-08-28), capturado 2026-09-02. §16: `/context_merge` desde `contexto_vivo/`, reunión "Join Soporte Clientes" (2026-09-02), capturado 2026-09-02. §14.5: `/context_merge` desde `contexto_vivo/`, mail "RE: Version W 73 Wallet Service" (Fintexa, 2026-09-08), capturado 2026-09-09. §17: `/context_merge` desde `contexto_vivo/`, reunión "ARDID" (2026-09-09), capturado 2026-09-10. §18: `/context_merge` desde `contexto_vivo/`, sesión `/idea_solution` sobre `ardid_desconocimientos` (PRD-248), capturado 2026-09-17. §19: `/context_merge` desde `contexto_vivo/`, sesión `/idea_solution`+`/idea_us` de `ardid_limites_pj`, capturado 2026-09-23.*
