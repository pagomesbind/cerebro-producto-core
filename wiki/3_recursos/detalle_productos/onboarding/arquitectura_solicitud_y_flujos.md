...
- **Casos legado que se mantienen fuera de esta migración:** casos existentes bajo el modelo antiguo (ej. Tin y ciertos comercios) siguen operando tal cual están — el desarrollo nuevo no busca migrarlos, para poder concentrarse en los procesos nuevos (reunión "Producto", 2026-08-18 — Luciana Rudaz, Pablo Gomes, Emma Vignoles, Nicolás Colón).
...
### 1ter. Vulnerabilidad detectada (2026-08-25) — Renaper no compara imágenes de DNI, valida por texto

> Estado: en producción — hallazgo confirmado con una prueba real en ambiente productivo, no una hipótesis de diseño.

Durante una sesión de pruebas de onboarding en ambiente **productivo** (test conjunto entre Bind PSP y una auditora externa contratada por Banco Industrial, en el marco de la revisión "REI" — ver contexto de la tarea en `1_proyectos/tareas.md` T-014/T-024 del Cerebro de Pablo Gomes; el motivo de mover la prueba a producción fue una falla del proveedor de prueba de vida en STG), el equipo probó deliberadamente combinaciones de documentos para entender los límites reales de la validación:

- **Mecánica confirmada (aclarada por Emma Vignoles):** el sistema **no valida la imagen del DNI contra Renaper** — Renaper Datos consulta la API del organismo usando **CUIL + género + número de trámite** (campos de texto extraídos/ingresados, no la foto del documento). La validación biométrica (que la persona frente a cámara coincide con la foto del DNI) corre por un camino completamente aparte, vía el proveedor Socialnet/Facetech (prueba de vida, ver §6).
- **Prueba realizada:** se inició un registro cargando el frente del DNI de una persona y el dorso del DNI de una persona distinta, seguido de una selfie/prueba de vida — repitiendo combinaciones (frentes, dorsos, hasta tres personas distintas) para mapear qué bloquea el sistema y qué no.
- **⚠️ Vulnerabilidad identificada:** como consecuencia de ese diseño, el sistema permite en ciertas configuraciones **crear una cuenta combinando el frente y el dorso de DNIs de dos personas distintas** — nada en el flujo cruza la imagen del frente contra la del dorso, ni contra el resultado de Renaper más allá de los 3 campos de texto. La auditora externa (Victoria Farías, LLYASOC) señaló explícitamente que esto es una debilidad a incluir en el informe de auditoría.
- **Mitigación probada en la misma sesión (no corrige el hallazgo de fondo):** ante los bloqueos repetidos por intentos fallidos, se ajustó en caliente el parámetro de intentos fallidos permitidos a 4 — es un dato operativo (el límite de intentos es configurable), no una corrección del problema de fondo.
...
**Tres escenarios operativos identificados (reunión "Producto", 2026-08-18):** sin onboarding, con onboarding propio, y configuraciones combinadas donde el sistema debe poder identificar datos faltantes (ej. una selfie) y permitir al usuario completar el proceso en etapas — el mismo requisito de pausar/reanudar de arriba, visto desde el lado de qué combinaciones de configuración tiene que soportar.

**Endpoints propuestos:** para esto se proponen cuatro endpoints que gestionan creación y actualización de solicitudes de onboarding. Pablo Gomes buscaba convencer a Cristian (proveedor) de permitir que los procesos en espera se completen vía **PATCH**, en lugar de requerir reinicios completos del flujo.
...
### 6.1 Prueba de vida y concordancia facial son dos validaciones distintas — la norma exige las dos

Aclaración de Pablo Gomes (reunión "Producto", 2026-09-08), que hasta ahora no estaba explicitada en esta wiki: **prueba de vida** y **concordancia facial (face match)** son dos validaciones biométricas distintas, y la normativa exige tener **las dos**, no una sola.

- **Prueba de vida ("liveness"):** responde solo "vive o no vive" — no tiene score ni puntaje de coincidencia. Confirma que la persona frente a la cámara es una persona real en el momento (no una foto, no un video pregrabado).
- **Concordancia facial (face match):** mide qué tan parecida es la persona a la foto del DNI que se está validando — sí tiene un score/puntaje de similitud.

Ninguno de los proveedores relevados a la fecha entrega el video completo como evidencia — lo que sí entregan ambos (Socialnet, FaceTech) es una **captura 2D** (foto) extraída del video de la prueba de vida, que se cruza contra la foto del DNI. **Socialnet** hace ambas validaciones combinadas en un mismo servicio: agarra el video, extrae 3 imágenes, compara una contra la imagen del DNI (face match) y manda la otra a RENAPER (para cruzar contra el rostro registrado ahí).

**Implicancia para el diseño de evidencia del legajo (PRD-147/PRD-202):** el paquete de evidencia armado por Onboarding para cada solicitud debe distinguir y dejar constancia de las dos validaciones por separado (prueba de vida: vive/no vive, sin score; face match: score de similitud + foto comparada), no tratarlas como una sola pieza de evidencia genérica "biometría". Caso que disparó la aclaración: María Victoria Simonetti (PLD, Banco Industrial) había pedido que la evidencia de prueba de vida incluya explícitamente el score del proveedor y el motivo por el cual se dejó pasar (o no) la prueba — pedido que en rigor corresponde al *face match*, no a la prueba de vida en sí (que no tiene score).

> Fuente: reunión "Producto" (2026-09-08), Pablo Gomes explicando a Emma Vignoles. Capturado 2026-09-09.
...
**⚠️ Gap — actualización posterior (reunión "Producto", 2026-08-18) con orden distinto, no reconciliado con la lista de arriba:** en esa reunión (Luciana Rudaz, Pablo Gomes, Emma Vignoles, Nicolás Colón) se repriorizó como personas físicas primero, luego **personas jurídicas**, y por último **menores** — invirtiendo el orden de los puntos #2/#3 de la lista original (2026-07-22, donde menores va antes que jurídicas). Comercios se reafirma como última prioridad en ambas fuentes, priorizando una base robusta del alta de cuentas antes de expandir alcance. No está confirmado si esta reunión reemplaza formalmente el orden anterior o si es una inconsistencia sin resolver entre ambas fuentes.
...
**Riesgo relacionado, señalado en la reunión "Producto" (2026-08-18):** preocupación por terminar con una arquitectura difícil de mantener ("Frankenstein") al tener servicios separados para los clientes que usan el onboarding interno de Bind PSP vs. los que usan uno propio (ver los tres escenarios operativos de §3 y los casos legado de §0). Se reconoce la dificultad en la propia reunión, pero se decide avanzar igual con esta implementación como punto de partida.

---
*Última actualización: 2026-09-08 — `/context_merge`: nueva §6.1 — prueba de vida y concordancia facial (face match) son dos validaciones biométricas distintas, la norma exige ambas; implicancia para el diseño de evidencia del legajo de PRD-147/PRD-202.*
