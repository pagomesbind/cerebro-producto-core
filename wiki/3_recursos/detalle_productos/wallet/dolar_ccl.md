...
- **Compra fallida con `ComisionInterna = 0` no se devolvía** ([WS-1351](https://bindpsp.atlassian.net/browse/WS-1351), W 72, 3 SP, Epic WS-79): en una compra de dólar CCL, si `ComisionInterna` era $0 y la operación era rechazada por Poincenot en la pata 1, el intento de devolución fallaba — `MS Comprobantes` rechazaba crear un comprobante de cargo por un monto de $0 y el flujo de devolución se cortaba ahí (el cliente no recibía su dinero de vuelta). **Fix:** cuando la comisión a aplicar es $0, directamente no se genera comprobante de cargo — el flujo de devolución continúa sin ese paso. Aplica tanto a compra como a venta de dólar CCL. Validado por Nicolás Colón el 2026-08-04. Es el ticket referido (sin nombrar) en §3.6bis.
...
**Cierre (2026-08-26):** el ticket referido en el mail es **[WS-1351](https://bindpsp.atlassian.net/browse/WS-1351)** (compra fallida con `ComisionInterna = 0` que no se devolvía) — publicado y corregido en W 72. Ver el detalle técnico completo en §3.6.
