...
**Reintento automático adicional en la creación de alias ante error de APIBank (2026-08-31, confianza media) — confirmado con ticket concreto (2026-09-02).** En la reunión "Weekly - Producto / Operaciones" (2026-08-31), Pablo Gomes informó que se implementó un reintento automático en la creación de alias de cuenta cuando la operación recibe un error de APIBank — descripto como mitigación de un error frecuente y como una regresión nueva ("antes andaba bien"), no un error histórico. Es un reintento **distinto** del fix de 700ms de MDA-292391/Banco Industrial documentado arriba — mismo tipo de problema (falla de asignación de alias ante error de APIBank), mecanismo de reintento separado.

**Confirmación técnica (reunión "Analisis de Riesgo - Emisión V 72.2", 2026-09-02, tickets WS-1556/DEM-1828):** el mecanismo es un **reintento automático con espera fija corta**, cubriendo tanto el flujo automático de creación de CBU como el endpoint de asignación directa de alias (mismo motivo de fondo en ambos casos — Coelsa puede demorar en registrar la CBU nueva en la parte de su sistema que resuelve la consulta previa del banco emisor, devolviendo "CBU no existe" y dejando la cuenta con alias nulo/bloqueado). Semáforo asignado: amarillo, con seguimiento post-implementación a cargo de Gonzalo Rivera (monitorea que baje la cantidad de altas de CBU corta que quedan con alias nulo — problema activo con reclamos de clientes al momento de esta confirmación). Parte del lote de despliegue **V72.2** (incluye también regularización de cuenta corriente/movimientos WS-1554 y conciliación de cash out WS-1552), reprogramado del jueves 2026-09-03 al lunes 2026-09-07 8:00hs por falta de tiempo de pruebas y política de Fintexa de no desplegar los viernes. Mismo problema de fondo ya trackeado en `1_proyectos/asignacion_alias_cvu/` (PRD-224) y tarea T-032 de `tareas.md` — confirma que el fix avanzó a ticket de desarrollo concreto y está en camino de despliegue.
...
---
*Última actualización: 2026-09-03 — `/context_merge`: confirmación técnica del reintento de alias con tickets WS-1556/DEM-1828 y detalle de despliegue V72.2 (§1).*
*Última actualización anterior: 2026-09-02 — `/context_merge`: reintento automático de alias ante error de APIBank (§1) y nueva §3 con nota operativa de baja confianza sobre requisitos regulatorios 2024 (domicilio real, consulta de actividad AFIP).*
