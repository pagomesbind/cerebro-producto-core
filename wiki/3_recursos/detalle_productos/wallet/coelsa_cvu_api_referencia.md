# Referencia técnica — API CVU de Coelsa

> Estado: en producción (documentación de referencia — Bind opera contra esta API; no es la API pública de Bind). Fuente: documentación pública de Coelsa (VPN habilitada), https://documentacion.coelsa.com.ar/cvu/#introduccion, sitio completo. Consultada 2026-09-11.
>
> **Distinción importante:** [wallet/apis_expuestas/cvu/](apis_expuestas/cvu/) documenta la **API pública que Bind expone** a sus clientes (`POST /cvu`, `PATCH /alias`, etc. — dominio exclusivo de `/sync_web`, no tocar desde acá). Este archivo documenta la capa de abajo: **la API real de Coelsa** que el backend de Bind consume para que esa API pública funcione (alta/baja de PSP, cuentas recaudadoras, CVU en sí, comercios).

## 1. Autenticación y ambientes

Mismo esquema OAuth2 *password grant* que DEBIN (ver [coelsa_debin_api_referencia.md §1](coelsa_debin_api_referencia.md)), contra un endpoint de auth **v2** propio de CVU. Tres ambientes:
...
- Complementa directamente [validaciones_y_alias_cvu.md](validaciones_y_alias_cvu.md) y `wallet/apis_expuestas/cvu/` (API pública de Bind) — esta es la capa de implementación por debajo.
...
- El alta masiva por SFTP (10.000 CVU o comercios por archivo) no estaba documentada en ningún lugar de la wiki — puede ser relevante para iniciativas de altas masivas (cross-ref: el proyecto PRD-66/Provincianet trabajó un problema de volumen de altas de QR; vale la pena revisar si el canal SFTP masivo de Coelsa es una alternativa a lo investigado ahí).
- Ver también [coelsa_debin_api_referencia.md](coelsa_debin_api_referencia.md) para la superficie de API Bancos compartida y el mecanismo de scoring/Prevent común a DEBIN y CVU.
