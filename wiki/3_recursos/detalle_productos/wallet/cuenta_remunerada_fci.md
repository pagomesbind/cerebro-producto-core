...
**Defectos de contrato abiertos al momento de esta ingesta** (todos en estado `EN QA`/`Asignado`, ninguno con fix confirmado):
- No valida que la cuenta esté habilitada para FCI antes de responder ([WS-1317](https://bindpsp.atlassian.net/browse/WS-1317)).
- El parámetro `Sort` es case-sensitive (solo acepta `ASC`/`DESC` en mayúsculas) ([WS-1306](https://bindpsp.atlassian.net/browse/WS-1306)).
- `IdCuentaComitente` no valida que sea numérico (acepta strings alfabéticos) ([WS-1305](https://bindpsp.atlassian.net/browse/WS-1305)).
- Inconsistencia de formato de fecha (`YYYYMMDD` en este endpoint vs. `YYYY-MM-DD` en el resto de la API de FCI) ([WS-1304](https://bindpsp.atlassian.net/browse/WS-1304)).
- Nomenclatura de parámetros mezclando PascalCase/camelCase y español/inglés (`IdCuentaComitente`, `From`, `To`, `Sort` vs. `fechaProceso`, `paso`) — se propuso unificar a camelCase en español para este endpoint ([WS-1303](https://bindpsp.atlassian.net/browse/WS-1303)).
