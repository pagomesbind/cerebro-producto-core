---
id: 2026-09-10_riesgo-controles-insuficientes-onboarding-gestionado-por-gallo
pm: nicolas
fecha_captura: 2026-09-10
fuente: "Reunión \"Join Soporte Clientes\" (2026-09-09, 10:03), minuta Gemini"
producto: onboarding
tema: Falta de controles detectada en los onboardings gestionados por el integrador Gallo — Terra Blockchain ya dado de baja, auditorías solicitadas para Bolsa de Comercio de Chaco, IEB y B Investment
tipo: riesgo
destino_propuesto: 2_areas/riesgos.md
tipo_destino: actualizar
contradice: "no"
confianza: alta
estado: en_cola
---

Emma Vignoles informó que ya se envió una **carta documento de baja a Terra BlockChain** (cliente con ficha en `casos_de_uso_clientes.md`, modelo white-label del ecosistema Gallo — "modelo simil IEB") y advirtió sobre una **falta de controles generalizada en los onboardings gestionados por el integrador Gallo**, que afecta también a **Bolsa de Comercio de Chaco, IEB y B Investment**. Se solicitaron auditorías inmediatas de onboarding para estos clientes al equipo de compliance (acción ya en próximos pasos: Emma Vignoles).

**Caso adicional discutido — B Investment:** Adriana Endzeliz y Emma Vignoles revisaron su volumen transaccional (4 operaciones en junio, 9 en julio por $72.000, nula actividad en agosto y septiembre); ante el desconocimiento de la operatoria real del cliente, se acordó agendar una reunión para examinar sus actividades.

**Gap de identificación:** de las 4 entidades mencionadas, **Bolsa de Comercio de Chaco** y **B Investment** no aparecen en `log_clientes.md` — posibles entidades nuevas del ecosistema white-label de Gallo (mismo patrón que Terra BlockChain e IEB, ya cargadas) todavía no relevadas en Notion. Queda para que `/sync_customers` las confirme en su próximo barrido.

> Fuente: Reunión "Join Soporte Clientes" (2026-09-09, 10:03), minuta Gemini.
