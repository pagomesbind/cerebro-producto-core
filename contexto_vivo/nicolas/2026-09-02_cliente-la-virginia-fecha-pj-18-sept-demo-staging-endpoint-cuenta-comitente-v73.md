---
id: 2026-09-02_cliente-la-virginia-fecha-pj-18-sept-demo-staging-endpoint-cuenta-comitente-v73
pm: nicolas
fecha_captura: 2026-09-02
fuente: "Reunión \"Join Soporte Clientes\" (2026-09-02)"
producto: onboarding
tema: Estado del desarrollo de Onboarding Personas Jurídicas para La Virginia — fecha de compromiso 18-sep, demo en staging planificada 14-16 sep, y endpoint de alta de cuenta comitente confirmado para la versión 73
tipo: conocimiento
destino_propuesto: 2_areas/clientes/casos_de_uso_clientes.md
tipo_destino: actualizar
contradice: "no"
confianza: media
estado: ingestado
merge_commit: d783db8
---

En la reunión "Join Soporte Clientes" (2026-09-02), Mauro Suppan consultó por el estado del desarrollo de Onboarding Personas Jurídicas para **La Virginia** (proyecto ya trackeado como `1_proyectos/proyecto-la-virginia-ob-pj/` — carpeta que no existe en la instancia de este PM, pertenece a otro Cerebro del equipo; se captura igual como novedad de cliente por si el PM dueño no asistió a esta reunión puntual). Matias Alzogaray se comprometió a: (1) confirmar si se mantiene la fecha de compromiso de salida a producción del **18 de septiembre**, y (2) evaluar habilitar un ambiente de Staging para que el cliente pueda validar antes del despliegue. Gonzalo Rivera sugirió realizar la demostración con el cliente entre el **14 y 16 de septiembre**, condicionado a que el desarrollo esté disponible en Staging para el 14/09. Se acordó como decisión de la reunión: realizar esa demostración en Staging con La Virginia antes del pase a producción.

Adicionalmente, Mauro Suppan preguntó por el desarrollo de un **endpoint para alta de cuenta comitente** (permite a clientes como La Virginia enviar el identificador de cuenta para crear la cuenta comitente). Matias Alzogaray confirmó que esta funcionalidad forma parte de la **versión 73** (septiembre), con las tareas técnicas 1424 y 1425 confirmadas y 1423 en evaluación de inclusión.

> Fuente: Reunión "Join Soporte Clientes" (2026-09-02), minuta Gemini. Si el PM dueño del proyecto `proyecto-la-virginia-ob-pj` no tiene registro de esta reunión, este item también sirve como aviso de la novedad vía `contexto_vivo/`.
