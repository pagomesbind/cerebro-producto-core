---
id: 2026-09-01_arquitectura_api_bank_transferencia_mep
pm: pablo
fecha_captura: 2026-09-01
fuente: "Portal público de developers API BANK — https://sandbox.bind.com.ar/apidoc/#api-TransferenciaMEP"
producto: transversal
tema: API BANK (Banco Industrial) — TransferenciaMEP — referencia de endpoints
tipo: conocimiento
destino_propuesto: 3_recursos/arquitectura_sistema/api_bank/transferencia_mep.md
tipo_destino: crear
contradice: "no"
confianza: alta
estado: ingestado
...
Grupo **TransferenciaMEP** de API BANK: transferencias MEP (Mercado Electrónico de Pagos) — operatoria bursátil/BCRA usada para suscripción/rescate de fondos comunes de inversión, cancelación de saldos deudores de mercado de capitales, y transferencias entre cuentas propias o de terceros (incluso en moneda extranjera). Es un grupo más especializado que `Transferencia`/`Billetera` — no está directamente ligado a CVU/CBU de Wallet o Agente de Cobros y Pagos, pero se incluye por relevancia transversal al dominio de transferencias de API BANK.

Header `Authorization: JWT :token` obligatorio en todos. `bank_id` siempre `322`, `view_id` siempre `"owner"`.
...
Mismo contrato que la creación (punto 1).
