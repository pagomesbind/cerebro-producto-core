---
id: 2026-09-01_arquitectura_api_bank_webhooks
pm: pablo
fecha_captura: 2026-09-01
fuente: "Portal público de developers API BANK — https://sandbox.bind.com.ar/apidoc/#api-Webhooks"
producto: transversal
tema: API BANK (Banco Industrial) — Webhooks — referencia de endpoints
tipo: conocimiento
destino_propuesto: 3_recursos/arquitectura_sistema/api_bank/webhooks.md
tipo_destino: crear
contradice: "no"
confianza: alta
estado: ingestado
...
Grupo **Webhooks** de API BANK: administración de las suscripciones de webhook del cliente (Bind, en este caso) — a qué URL y a qué eventos se notifica de forma asincrónica (transferencias recibidas, DEBIN acreditado/rechazado, etc.). Los payloads concretos de cada tipo de evento están documentados en el grupo `Eventos` (ver item separado `2026-09-01_arquitectura_api_bank_eventos`). Header `Authorization: JWT :token` obligatorio en todos.
...
| Parámetro | Tipo | Obligatorio | Descripción |
|-----------|------|-------------|-------------|
| `code` | String | Sí | Código único del webhook a eliminar. |
...
| `body.data` | Object | Sí | Contenido particular del webhook, variable según el tipo — ver ejemplos en grupo `Eventos` (`WebhookCreated`, `WebhookAcreditedDebin`, `WebhookRejectedDebin`, `WebhookRefundedDebin`, `WebhookTransferCvuReceived`, `WebhookTransferCbuReceived`). |
...
Grupo completo (4/4 endpoints relevados) desde `https://sandbox.bind.com.ar/apidoc/api_data.json`. El catálogo `Referencias-EventsEnpoints` (lista de eventos suscribibles) y el detalle de cada payload de evento pertenecen al grupo `Eventos` (ver item separado) y `Referencias` (fuera de alcance de esta ronda).
