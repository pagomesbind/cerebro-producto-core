---
id: 2026-09-01_arquitectura_api_bank_eventos
pm: pablo
fecha_captura: 2026-09-01
fuente: "Portal público de developers API BANK — https://sandbox.bind.com.ar/apidoc/#api-Eventos"
producto: transversal
tema: API BANK (Banco Industrial) — Eventos (payloads de webhooks) — referencia de endpoints
tipo: conocimiento
destino_propuesto: 3_recursos/arquitectura_sistema/api_bank/eventos.md
tipo_destino: crear
contradice: "no"
confianza: alta
estado: ingestado
...
Grupo **Eventos** de API BANK: no son endpoints que el cliente (Bind) invoque, sino la **documentación de los payloads** que el banco envía al webhook configurado por el cliente (ver grupo `Webhooks`, endpoint `AltaModificacionWebhook`). Cada uno de los 10 endpoints de este grupo documenta el `type` de evento y la forma de su `data`. Todos comparten el sobre (envelope) común:
...
Se envía al dar de alta la URL de un webhook, **para confirmar que quedó correctamente configurado**. Si este envío falla, se rechaza la creación del webhook (ver grupo `Webhooks`, `AltaModificacionWebhook`, sección "validación activa de la URL").
...
Informa una novedad en la cuenta de un cliente PSI/STI (ver grupo `Alta_De_Cuenta`): si fue creada o rechazada. `object: "ClientAccountUpdateEvent"`, `type: "client.account.updated"`.
...
`object: "TransferEventMep"`, `type: "transfer.mep.completed"`. Mismo patrón `credit`/`debit` que `WebhookTransferCbuReceived`, más los campos propios de MEP.

### `data` — campos adicionales sobre el patrón credit/debit

`transfer_type: "TRANSFER-MEP"`, `net_id` (**id generado por el BCRA** — corresponde al `bcra_operation_number` que devuelven `ObtenerTransferenciaMEP`/`ObtenerTransferenciasMEP`), `net_name` (default `BCRA`), `status` (default `COMPLETED`), `status_description`, `mep_operation`, `observation`, `investment_type`, `origin_fund`, `creditor_entity`, `creditor_account`, `description`.
