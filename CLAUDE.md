...
1. **Detección Activa de Gaps y Contradicciones:**
   - Un gap es puntualmente una inconsistencia sobre el **conocimiento ya documentado** del Cerebro: dos fuentes que se contradicen entre sí, una definición ambigua ya escrita, o una norma/dato que no cierra con lo que ya está en la wiki. Detectalo de forma proactiva durante la lectura o procesamiento de cualquier información (en `raw/` o fuentes externas).
   - **No es un gap que a un proyecto le falte algo por definir o que reste saber.** Eso significa que el PM todavía no lo consultó, negoció o pimponeó con el stakeholder correspondiente — y esa gestión es tarea del PM, no un hallazgo pasivo del Cerebro. En ese caso, creá de inmediato una tarea en `wiki/1_proyectos/tareas.md` (punto 3) con una sugerencia de con quién definirlo, nunca un gap.
   - NO asumas ni inventes la solución de un gap real. Registralo obligatoriamente estructurado por fecha, nivel de severidad (Alta/Media/Baja) y descripción de la contradicción, en el destino correcto: `gaps.md` del proyecto/IDEA si es específico de uno (directo), o un item `tipo: gap` en `contexto_vivo/` si es de contexto fijo (nunca directo a `wiki/2_areas/gaps_y_preguntas.md`).
   - Al final de tu turno, debes consultar explícitamente al usuario en la terminal sobre cómo resolver los gaps reales que hayas detectado.
...
3. **Backlog de Tareas de Producto:**
   - Se registra acá únicamente lo que es puntualmente **trabajo de Producto del PM ligado a un proyecto** — actual o una posible IDEA nueva (ej. un stakeholder pide hacer discovery o estimar una iniciativa todavía sin trackear), o una definición pendiente de un proyecto que te resta ir a negociar, consultar o pimponear con un stakeholder (ver punto 1) — en `wiki/1_proyectos/tareas.md` (personal), con responsable, interesados, urgencia, fecha detectada, fecha límite si se conoce, fuente y estado. Dedupe obligatorio.
   - **No registres tareas operativas** aunque te involucren a vos y aparezcan en una fuente procesada (ej. coordinar con otro equipo una prueba técnica, un seguimiento administrativo, una gestión de acceso) — eso no es Producto y no suma al Cerebro.
