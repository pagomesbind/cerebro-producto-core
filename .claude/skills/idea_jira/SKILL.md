...
2. **Verificá si la descripción de la IDEA está vacía.** Es el caso típico de una IDEA creada en un discovery previo a que existiera un PRD formal (ej. al cierre de `/idea_start`) — nunca asumas que "ya existía" implica que ya tiene su contenido. Si está vacía, completala ahora con el PRD completo en markdown, autocontenido, tal como está en el artefacto (mismo criterio que el Paso 2A.1) — no la dejes vacía solo porque la IDEA ya existía. Si ya tiene una descripción distinta al PRD vigente (ej. un resumen corto del discovery original), mostrale el delta al PM y actualizala solo con su confirmación explícita (mismo criterio que la Regla dura 3).
...
4. Si la IDEA no está todavía en `EN APROBACION` ni en un estado posterior, transicionala (Regla dura 1). Si ya está en un estado posterior, no la toques — avisá.
