...
12. **Todo diagrama del artefacto de solución (o de las historias, si aplica) va embebido en la descripción correspondiente como bloque de código, marcado explícitamente como Mermaid** (ej. ` ```mermaid ` seguido del diagrama) — el conector no soporta subir archivos, así que esta es la única forma de que el diagrama llegue al desarrollador. Nunca omitas un diagrama que el artefacto tenga, y nunca lo menciones sin incluirlo ("ver diagrama en la wiki" no vale acá).
13. **Nada se crea en Jira sin que el PM vea antes qué se va a crear/actualizar y dé su OK explícito** — es una acción visible para todo el equipo (comité de aprobación, Ingeniería), no una escritura silenciosa como el resto de esta wiki.
14. Todo output en español.
